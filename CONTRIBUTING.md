...
Preserve the public `discover()` API and `svft.discovery/v1` contract. Changes to URL normalization, provenance, endpoint identity, sanitization, HTTP semantics, resource limits, or CLI behavior need focused tests and documentation. Public and persisted URL output must redact sensitive query parameter values without changing crawler request URLs or SecurityTarget identity.
